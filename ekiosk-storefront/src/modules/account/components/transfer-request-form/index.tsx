  ...
  return (
    <div className="flex w-full flex-col gap-y-4">
      <div className="grid w-full items-center gap-x-8 gap-y-4 sm:grid-cols-2">
        ...
        <form action={formAction} className="flex flex-col gap-y-1 sm:items-end">
          <div className="flex w-full flex-col gap-y-2">
            ...
            <SubmitButton variant="secondary" className="w-fit self-end whitespace-nowrap">
              Request transfer
            </SubmitButton>
          </div>
        </form>
      ...
      {!state.success && state.error && (
        <Text className="text-base-regular text-right text-rose-500">{state.error}</Text>
      )}
      {showSuccess && (
        <div className="flex w-full items-center justify-between self-stretch bg-neutral-50 p-4 shadow-borders-base">
          <div className="flex items-center gap-x-2">
            ...
            <div className="flex flex-col gap-y-1">
              <Text className="text-medim-pl text-neutral-950">Transfer for order {state.order?.id} requested</Text>
